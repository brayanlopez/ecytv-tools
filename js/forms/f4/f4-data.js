...
export function restoreFormData(data, tbody, form) {
  ...
  salas.forEach((sala, idx) => {
    if (idx === 0 && firstRow) {
      ...
      row.innerHTML = `
        <td data-label="Sala"><input type="text" name="sala-nombre" placeholder="Sala" list="salas-sugeridas" value="${sala.nombre || ""}" aria-label="Sala adjudicada"></td>
        <td data-label="Fecha"><input type="date" name="sala-fecha" required value="${sala.fecha || ""}" aria-label="Fecha"></td>
        <td data-label="Inicio"><input type="time" name="sala-hora-inicio" required value="${sala["hora-inicio"] || ""}" aria-label="Hora de inicio"></td>
        <td data-label="Finalización"><input type="time" name="sala-hora-fin" required value="${sala["hora-fin"] || ""}" aria-label="Hora de finalización"></td>
