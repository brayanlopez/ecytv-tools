...
export function restoreFormData(data, tbody, form) {
  ...
  equipos.forEach((eq, idx) => {
    if (idx === 0 && firstRow) {
      ...
      row.innerHTML = `
        <td><input type="text" class="item-num" name="equipo-item" placeholder="Item" value="${eq.item || ""}" aria-label="Número de item"></td>
        <td><input type="text" name="equipo-tipo" placeholder="Tipo" required value="${eq.tipo || ""}" aria-label="Tipo"></td>
        <td><input type="number" name="equipo-cantidad" placeholder="Cantidad" required value="${eq.cantidad || ""}" aria-label="Cantidad" min="1"></td>
        <td><input type="text" name="equipo-codigo" placeholder="Código" required value="${eq.codigo || ""}" aria-label="Código"></td>
        <td><input type="text" name="equipo-elemento" placeholder="Elemento" required value="${eq.elemento || ""}" aria-label="Elemento"></td>
