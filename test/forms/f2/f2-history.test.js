...
describe("F2 History", () => {
  ...
  describe("History", () => {
    it("should save and restore history entries", () => {
      ...
      const stored = JSON.parse(localStorage.getItem("f2-history"));
    ...
    it("should delete history entries", () => {
      document.getElementById("nombre").value = "Juan Pérez";
      document.getElementById("btn-save").click();
      expect(JSON.parse(localStorage.getItem("f2-history"))).toHaveLength(1);

      const deleteBtns = document.querySelectorAll(".btn-history-delete");
      deleteBtns[0].click();

      expect(JSON.parse(localStorage.getItem("f2-history"))).toHaveLength(0);
